...
自动触发检测会按服务使用智能频率控制：服务持续可用时，最短自动检测间隔会逐步延长；检测失败时会逐步缩短。间隔范围为 5 秒到 30 分钟。手动切换节点和 `c` 手动检测会立即执行一次检测，不受该间隔限制。`a` 开关按当前服务单独控制。按 `t` 添加任务时，会从当前 Clash ProxyGroup 和尚未配置的服务中选择，并保存到配置文件。
